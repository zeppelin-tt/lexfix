...
Кириллица не трогается ни на одном слое: правятся только латинские токены.
...
class Corrector:
    ...
    def _resolve(self, phrase: str, context: str) -> str | None:
        if translit.has_cyrillic(phrase):
            ...
            return fixed if fixed and fixed != phrase else None
    ...
    def suggest_variants(self, phrase: str, limit: int = 8) -> list[tuple[str, str]]:
        ...
        if self.llm_enabled:
            out += [(g, "догадка")
                    for g in self._llm_variants(phrase, [t for t, _ in out])]
    ...
    def _llm_variants(self, phrase: str, known: list[str]) -> list[str]:
